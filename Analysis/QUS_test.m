...
for i = 1:length(samples)
    ...
    anatomy_array = table2array(anatomy_table(1,2:end));
    ...
    if Segment
        repro.segment_repro_frames(mat_file,anatomy_array);
    end
    
    %Compute the QUS parameters in the entire image
    if ProcessQUS
        repro.qus_processing(mat_file,anatomy_array,qus_file);
    end
    
    %Create figures displaying the parameter maps
    if OverlayQUS
        repro.plot_bmode_qus_overlay(qus_file,anatomy_array);
    end
