function qus_processing_Burr(in_fid,qus_frames,qus_save_str)
...
%                     xlim([sub_f(1)-0.2e6, sub_f(end)+0.2e6]./1e6)
                ...
                % Computer Burr b parameters
                [b_fit, lambda_fit] = BurrFit(samp.env_sub_vol);
                result{param2idx(params,'Burr_b')}(k,i,j) = b_fit;
                result{param2idx(params,'Burr_lambda')}(k,i,j) = lambda_fit;
