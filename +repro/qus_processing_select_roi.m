function qus_processing_select_roi(in_fid,qus_frames)
...
%                     %% Gaussian Form Factor (ref phantom method)
                ...
                %%ADDED BY ANDREW  
                %Compute Burr parameters
                [b_fit, lambda_fit] = BurrFit(samp.env_sub_vol);
                %[N, X] = hist(Env,200);
                env_hist = histogram(samp.env_sub_vol(:),'NumBins',70,'Normalization','probability');
                N = env_hist.NumBins;
                DeltaX = env_hist.BinWidth;
                X = env_hist.BinEdges + DeltaX/2;
                %PDF_Env = N/sum(N)/DeltaX;
                
                Burr_fit = (1/lambda_fit^2)*2*X*(b_fit-1)./(((X/lambda_fit).^2+1).^b_fit);
...
%                 % Plot the histogram with the pdf fits
                    ...
                    hold on
                    plot(X,Burr_fit*DeltaX,'-.g','LineWidth',3)
                    %legend('Envelope Histogram','H-k Fit');
                    legend('Envelope Histogram','H-k Fit','Nakagami Fit','Burr');
                    ...
                    {'ESD', aeff; 'EAC', ac; 'H-k alpha', hk_scat_clust_param; 
                        ...
                        'ROI Z',roi.pos_z*1000; 'Burr-b', b_fit; 
                        'Burr_lambda', lambda_fit}
