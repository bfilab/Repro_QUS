function ESD_segment_zones(qus_results_fid,frame_num)
...
%ADDED BY ANDREW 
%Threshold for binary separation of labyrinth and junctional zones
thresh = 27;

top_dir = cd;
date_str = char(datetime(datetime,'Format','yyyy-MM-dd'));
%date_str = datestr(datetime,'yyyy-mm-dd');
%date_str = '2023-06-01-centered'
%date_str = '2023-07-12';
...
qus_params = {'Effective Scatterer Size'};
...
% qus_str = {'HK-alpha','HK-k','Naka-m','log10(Naka-Omega)','SS','I0','MBF','ESD','EAC'};
qus_str = {'ESD-Manual-Seg'};
...
%     scatter(xm(in_roi_idx).*1000,zm(in_roi_idx).*1000,'.r');
    ...
    if strcmp(qus_params_to_plot{p_count},'Effective Scatterer Size')
        ...
        %this_p_map(this_p_map < thresh) = min(qus_clims{p_count});
        %this_p_map(this_p_map > thresh) = max(qus_clims{p_count});
...
%         this_cmamp = interp1([0;1],qus_cmap{p_count},linspace(0,1,256));
    ...
    %ADDED BY ANDREW
    %Draw new segmentation boundary around Labyrinth, including a buffer
    %zone around fetal side of the placental ROI
    l_manual = drawpolygon(gca,'Color','k');

    %Labyrinth zone is the area included in both p_roi and l_roi
    %Find the intersection of the placenta roi and the labyrinth roi using
    %Must convert to polyshapes to use intersect function
    p_pshape = polyshape(seg_struct(1).p_roi.Position); 
    l_pshape = polyshape(l_manual.Position);
    l_pshape = intersect(l_pshape,p_pshape);

    %Junctional zone is the area in p_roi that is not in l_roi
    %Find which part of the placenta roi does not include the labyrinth,
    j_pshape = xor(l_pshape,p_pshape);

    %Save labyrinth and junctional zone rois as polygons instead of
    %polyshapes to be consistent with the placenta roi datatype
    seg_zones.l_roi = drawpolygon('Position',l_pshape.Vertices)
    seg_zones.j_roi = drawpolygon('Position',j_pshape.Vertices)

    %Append the segmented zones to the RF data file    
    save(in_fid,'seg_zones','-append');
