function [all_res_table] = JZ_LZ_roi_analysis(qus_results_fid,frame_num)
...
if AddLines
    copy_dir = [cd,'\Line_Bmode_with_Zone_Overlay'];
else
    copy_dir = [cd,'\',date_str,'_Bmode_with_Zone_Overlay'];
end
mkdir(copy_dir);
...
% Load placenta zone segmentation data
lz_poly = seg_zones(frame_num).l_roi.Position; 
jz_poly = seg_zones(frame_num).j_roi.Position; 

% For compatiblity with the rest of the code, convert the units of the
% segmentation points from [mm] to [m]
lz_poly = lz_poly./1000;
jz_poly = jz_poly./1000;
...
% Find which corners are in the labyrinth zone roi
in_roi_idx = inpolygon(xm(:),zm(:),lz_poly(:,1),lz_poly(:,2));
in_roi_idx12 = inpolygon(xm2(:),zm2(:),lz_poly(:,1),lz_poly(:,2));
in_roi_idx21 = inpolygon(xm3(:),zm3(:),lz_poly(:,1),lz_poly(:,2));
in_roi_idx22 = inpolygon(xm4(:),zm4(:),lz_poly(:,1),lz_poly(:,2));

%Only include ids where all 4 corners fit within the roi
in_roi_idx = in_roi_idx & in_roi_idx12 & in_roi_idx21 & in_roi_idx22;
lz_sum = sum(in_roi_idx);
in_roi_idx_lz = find(in_roi_idx);

% Find which corners are in the junctional zone roi
in_roi_idx = inpolygon(xm(:),zm(:),jz_poly(:,1),jz_poly(:,2));
in_roi_idx12 = inpolygon(xm2(:),zm2(:),jz_poly(:,1),jz_poly(:,2));
in_roi_idx21 = inpolygon(xm3(:),zm3(:),jz_poly(:,1),jz_poly(:,2));
in_roi_idx22 = inpolygon(xm4(:),zm4(:),jz_poly(:,1),jz_poly(:,2));

%Only include ids where all 4 corners fit within the roi
in_roi_idx = in_roi_idx & in_roi_idx12 & in_roi_idx21 & in_roi_idx22;
jz_sum = sum(in_roi_idx);
in_roi_idx_jz = find(in_roi_idx);
...
hold on;
placenta.plot_roi(gca,jz_poly*1000);
placenta.plot_roi(gca,lz_poly*1000);
set(gca,'FontSize',16);
...
all_res_table = table;
all_res_table.Zone = {'Labyrinth';'Junctional'};
all_res_table.ID = {in_fid(1:end-7); in_fid(1:end-7)};
all_res_table.Area = [lz_sum; jz_sum];
...
%     scatter(xm(in_roi_idx).*1000,zm(in_roi_idx).*1000,'.r');
    ...
    %Define alpha maps for the two rois
    lz_alpha_map = zeros(size(this_p_map));
    %this_alpha_map(in_roi_idx) = 0.4;
    lz_alpha_map(in_roi_idx_lz) = 1;
    
    lz_alpha_map(this_p_map == 0) = 0;
    lz_alpha_map(isnan(this_p_map)) = 0;


    jz_alpha_map = zeros(size(this_p_map));
    %this_alpha_map(in_roi_idx) = 0.4;
    jz_alpha_map(in_roi_idx_jz) = 1;
    
    jz_alpha_map(this_p_map == 0) = 0;
    jz_alpha_map(isnan(this_p_map)) = 0;
    ...
    imagesc(new_ax,(all_roi_x+all_roi.len_x/2)*1000,(all_roi_z+all_roi.len_z/2)*1000,...
        this_p_map,'AlphaData',lz_alpha_map);
    axis(new_ax,'image');
...
%         this_cmamp = interp1([0;1],qus_cmap{p_count},linspace(0,1,256));
    ...
    new_ax = axes;
    imagesc(new_ax,(all_roi_x+all_roi.len_x/2)*1000,(all_roi_z+all_roi.len_z/2)*1000,...
        this_p_map,'AlphaData',jz_alpha_map);
        axis(new_ax,'image');
    rg_cmap = interp1([0;1],[1 0 0; 0 1 0],linspace(0,1,256));
%         this_cmamp = interp1([0;1],qus_cmap{p_count},linspace(0,1,256));
    colormap(new_ax,isolum(256,'invert',false));
    ...
    %Select and analyze labyrinth data
    lz_p_map = this_p_map(in_roi_idx_lz);
...
%     
    ...
    lz_p_map = lz_p_map(:);
    this_nan = isnan(lz_p_map);
    
    this_remove_N = 0;
    this_remove_N = this_remove_N + sum(this_nan);
    lz_p_map(this_nan) = [];
    
    this_zero = find(lz_p_map == 0);
    this_remove_N = this_remove_N + length(this_zero);
    lz_p_map(this_zero) = [];

    % Remove any Inf values (particular issue for log10(naka omega)
    lz_p_map = lz_p_map(~isinf(lz_p_map));

    lz_mean = mean(lz_p_map,'omitnan');
    lz_std = std(lz_p_map);

    %Select and analyze labyrinth data
    jz_p_map = this_p_map(in_roi_idx_jz);

    % Any pixel with value 0 or NaN should be removed
    jz_p_map = jz_p_map(:);
    this_nan = isnan(jz_p_map);
    
    this_remove_N = 0;
    this_remove_N = this_remove_N + sum(this_nan);
    jz_p_map(this_nan) = [];
    
    this_zero = find(jz_p_map == 0);
    this_remove_N = this_remove_N + length(this_zero);
    jz_p_map(this_zero) = [];

    % Remove any Inf values (particular issue for log10(naka omega)
    jz_p_map = jz_p_map(~isinf(jz_p_map));

    jz_mean = mean(jz_p_map,'omitnan');
    jz_std = std(jz_p_map);
    
    all_res_table.(qus_params_to_plot{p_count}) = [lz_mean; jz_mean];
    all_res_table.(strcat(qus_params_to_plot{p_count},'_std')) = [lz_std; jz_std];
...
%     this_qus_cell{p_count} = qus_mean(this_p_idx);
    ...
    save_str = sprintf('%s\\bmode_with_zone_overlay_%s',copy_dir,qus_str{p_count});
    saveas(gcf,save_str,'png');
    saveas(gcf,save_str,'fig');
% 
